...
public class GoogleMapsLocationsWebAppImpl implements GoogleMapsLocationsInterface {
    ...
    List<PlacesSearchResult> googleMapsPlaceSearchFind(String[] places) {
        ...
        for (String string : places) {
            try {
                ...
                logger.info("result from google maps place search {}", result);
                results.add(result);
            } catch (Exception e) {
    ...
    public GoogleRenderDirectionsPOJO provideRenderingServiceInformation(DirectionsPojo directionsPojo) {
        ...
        if (userData != null && userData.getKeywordsLikes() != null && userData.getKeywordsLikes().size() > 0) {
            StringBuilder sb = new StringBuilder();
            for (Keywords s : userData.getKeywordsLikes()) {
                sb.append(s);
                sb.append(", ");
            }
        ...
        DirectionsWaypoint[] waypoints = new DirectionsWaypoint[chatGptRecommendationsGoogleInfo.size()];
        logger.info("waypoints info generated from openAI suggestions generated by google {} ", chatGptRecommendationsGoogleInfo);
        for (int i = 0; i < waypoints.length; i++) {
            if (chatGptRecommendationsGoogleInfo.get(i) == null || chatGptRecommendationsGoogleInfo.get(i).placeId == null) {
                continue;
            }
            DirectionsWaypoint point = new DirectionsWaypoint();
